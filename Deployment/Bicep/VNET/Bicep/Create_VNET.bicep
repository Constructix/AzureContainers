...
param summarizedGatewayPrefixes object = {
  addressPrefixes: []
}
...
param firewallPublicIPAddressServiceTag string = ''
...
param bastionPublicIpAddressServiceTag string = ''
...
param natGatewayPublicIpAddressesNew array = []
...
param avnmResourceGroup string = '##avnm-resource-group-not-set##'
param avnmSubscriptionId string = '##avnm-subscription-id-not-set##'
...
var natGatewayTags = (contains(tagsByResource, 'Microsoft.Network/natGateways')
  ...
  : {})
var summarizedGatewayPrefixesComputed = ((length(summarizedGatewayPrefixes.addressPrefixes) > 0)
...
resource virtualNetwork 'Microsoft.Network/virtualNetworks@2024-01-01' = {
  ...
  properties: {
    ...
    summarizedGatewayPrefixes: summarizedGatewayPrefixesComputed
  }
  dependsOn: [
    // ... your other dependsOn items
  ]
}

// ==================== DDoS Protection Plan ====================
resource ddosProtectionPlan 'Microsoft.Network/ddosProtectionPlans@2020-11-01' = if (ddosProtectionPlanIsNew && ddosProtectionPlanEnabled) {
...
resource firewallPublicIpAddress 'Microsoft.Network/publicIPAddresses@2020-11-01' = if (firewallPublicIpAddressIsNew && firewallEnabled) {
  ...
  properties: {
    ...
    ipTags: ((firewallPublicIPAddressServiceTag != '')
      ? [
          {
            ipTagType: 'FirstPartyUsage'
            tag: firewallPublicIPAddressServiceTag
          }
        ]
      : [])
  }
...
resource natGatewaysWithNewPublicIpAddress_name 'Microsoft.Network/natGateways@2020-11-01' = [
  for item in natGatewaysWithNewPublicIpAddress: if (length(natGatewaysWithNewPublicIpAddress) > 0) {
    ...
    properties: item.properties
    dependsOn: [
      natGatewayPublicIpAddressesNew_name
    ]
  }
]

resource natGatewayPublicIpAddressesNew_name 'Microsoft.Network/publicIPAddresses@2020-11-01' = [
  for item in natGatewayPublicIpAddressesNew: if (length(natGatewayPublicIpAddressesNew) > 0) {
    name: item.name
    location: location
    sku: standardSku
    tags: publicIpAddressesTags
...
resource bastionPublicIpAddress 'Microsoft.Network/publicIPAddresses@2022-07-01' = if (bastionEnabled && bastionPublicIpAddressIsNew) {
  ...
  properties: {
    publicIPAllocationMethod: 'Static'
    ipTags: ((bastionPublicIpAddressServiceTag != '')
      ? [
          {
            ipTagType: 'FirstPartyUsage'
            tag: bastionPublicIpAddressServiceTag
          }
        ]
      : [])
  }
...
module avnmDeployment './nested_avnmDeployment.bicep' = if (length(ipamPools) > 0) {
  name: 'avnmDeployment'
  scope: resourceGroup(avnmSubscriptionId, avnmResourceGroup)
  params: {
